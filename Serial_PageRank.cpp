...
int main(int argc, char** argv) {
	...
	if (argc < 5) {
		printf("USAGE: <fileLocation> <numberOfNodes> <Tolerance> <Beta>\n");
		return 0;
	}
	...
	printf("Writing nodes and corresponding pagerank to outputSerial.txt\n");
	ofstream SerialFile("outputSerial.txt");

	for (int i = 0; i < N; i++) {
		SerialFile << "Node " << nodes->at(i).index << " has pagerank value : " << nodes->at(i).p_t1 << " \n";
	}
