...
int main(int argc, char** argv) {
	...
	if (argc < 5) {
		printf("USAGE: <file> <numberOfNodes> <Tolerance> <Beta>\n");
		return 0;
	}
	...
	while (error > threshold) {
		...
		int j, index, temp;
		#pragma omp parallel for private(index, temp, j) shared(nodes)
		for (i = 0; i < N; i++) {
			// Do not do anything for dead ends
			if (nodes->at(i).outLinkNo != 0) {
				temp = nodes->at(i).outLinkNo;
				for (j = 0; j < temp; j++) {
					index = indices[nodes->at(i).links.at(j)] - 1;
					#pragma omp atomic
		...
		double sum = 0.0;
		#pragma omp parallel for shared(nodes, sum)
		for (i = 0; i < N; i++ ) {
		...
		#pragma omp parallel for shared(nodes, error)
		for (i = 0; i < N; i++ ) {
	...
	printf("Writing nodes and corresponding pagerank to outputParallel.txt\n");
	ofstream ParallelFile("outputParallel.txt");

	for (int i = 0; i < N; i++) {
		ParallelFile << "Node " << nodes->at(i).index << " has pagerank value : " << nodes->at(i).p_t1 << " \n";
	}
